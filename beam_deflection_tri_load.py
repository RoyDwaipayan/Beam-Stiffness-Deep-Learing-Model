...
def mac(a,b):
    if a-b<0:
        ...
        return (a-b)
def deflection(x):
    R1 = W*total_length*(1-dist/total_length)
    y1 = R1/k1
    R2 = W*total_length - R1
    y2 = R2/k2
    #c2 = EI*y1
    ...
    defl = y1 + (dist/total_length)*(y2-y1) + (0.00651/(EI))*W*(total_length)**4
    return defl
#main
data_size = 10000
...
np.savetxt("beam_data_tri_load.csv",dataset,header="y1(0.5),k1,k2",comments='',delimiter=',')
